...
export default async function ManyWinPage({ params }: { params: Promise<{ id: string; date: string }> }) {
    ...
    matchesOnDate.forEach((match) => {
        if (match.winner1id) {
            winCounts.set(match.winner1id, (winCounts.get(match.winner1id) || 0) + 1);
        }
        if (match.winner2id) {
            winCounts.set(match.winner2id, (winCounts.get(match.winner2id) || 0) + 1);
        }
    ...
    // 4. 승리 수 기준으로 내림차순 정렬, 승률 계산 후 상위 5명만 자릅니다.
    const rankedPlayers = Array.from(winCounts.entries())
        .map(([pid, count]) => {
            const player = players.find((p: any) => p.id === pid);
            const totalGames = gameCounts.get(pid) || 0;
            const winRate = totalGames > 0 ? (count / totalGames) * 100 : 0;
            return { player, count, totalGames, winRate };
        })
        .filter((entry) => entry.player) // 선수 정보가 있는 경우만 필터링
