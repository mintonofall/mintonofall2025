...
export default function DashBoard() {
    ...
    const renderPlayerWithPhoto = (player: any) => {
        if (!player) return <div className="text-slate-400 w-12 text-center">-</div>;
        ...
        return (
            <div className="flex flex-col items-center justify-center gap-1 w-14">
                <div className="w-8 h-8 rounded-full bg-slate-200 overflow-hidden flex items-center justify-center flex-shrink-0 shadow-sm border border-slate-100">
                    {avatarSrc ? (
                ...
                <span className="text-[11px] font-medium text-slate-700 w-full truncate text-center" title={player.name}>
                    {player.name}
                </span>
            </div>
        );
    ...
    const renderContent = () => {
        switch (activeMenu) {
            case "user":
                return (
                    <div>
                        <p className="text-slate-500 mb-4">가입된 유저 목록과 보유 포인트를 표시할 수 있습니다.</p>
                        {isLoading ? (
                            ...
                            <div className="overflow-x-auto border rounded-lg shadow-sm">
                                ...
                                {users.length === 0 && (
                                    <div className="text-center py-10 text-slate-500">유저 데이터가 없습니다.</div>
                                )}
                            </div>
                        )}
                    </div>
            ...
            case "club":
                return (
                    <div>
                        <p className="text-slate-500 mb-4">생성된 클럽 목록과 설정된 코트 수 등을 표시할 수 있습니다.</p>
                        {isLoading ? (
                            <div className="text-center py-10 text-slate-500">데이터를 불러오는 중...</div>
                        ) : (
                            <div className="overflow-x-auto border rounded-lg shadow-sm">
                                ...
                                {clubs.length === 0 && (
                                    <div className="text-center py-10 text-slate-500">클럽 데이터가 없습니다.</div>
                                )}
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="overflow-x-auto border rounded-lg shadow-sm">
                                        ...
                                        {players.map((player) => (
                                            <tr key={player.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-center">
                                                    <div className="w-10 h-10 rounded-full bg-slate-200 overflow-hidden mx-auto flex-shrink-0">
                                                        {player.avater ? (
                                                            <img
                                                                src={
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-500 font-bold">
                                                    {player.grade}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-center text-slate-500">
                                                    {player.createdAt
                                ...
                                {players.length === 0 && (
                                    <div className="text-center py-10 text-slate-500">플레이어 데이터가 없습니다.</div>
                                )}
                            </div>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="overflow-x-auto border rounded-lg shadow-sm">
                                ...
                                {matches.length === 0 && (
                                    <div className="text-center py-10 text-slate-500">매치 데이터가 없습니다.</div>
                                )}
                            </div>
                        )}
                    </div>
            ...
            case "betting":
                return (
                    <div>
                        <p className="text-slate-500 mb-4">유저들의 베팅 내역과 적중 결과 등을 표시할 수 있습니다.</p>
                        {isLoading ? (
                            <div className="text-center py-10 text-slate-500">데이터를 불러오는 중...</div>
                        ) : (
                            <div className="overflow-x-auto border rounded-lg shadow-sm">
                                    ...
                                    <tbody className="bg-white divide-y divide-slate-200">
                                        {bettings.map((bet) => (
                                            <tr key={bet.id} className="hover:bg-slate-50 transition-colors">
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-center font-medium text-slate-900">
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-center">
                                                    {(() => {
                                                        ...
                                                        return <span className="text-slate-400">결과 대기</span>;
                                                    })()}
                                                </td>
                                                <td className="px-4 py-4 whitespace-nowrap">
                                                    <div className="flex justify-center items-center gap-2">
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-sm text-center">
                                                    {!bet.isProcess ? (
                                                        ...
                                                        <span className="px-2 py-1 bg-slate-200 text-slate-700 rounded-lg text-xs font-bold">
                                                            무승부 (환불)
                                                        </span>
                                                    ) : bet.isCorrect ? (
                                ...
                                {bettings.length === 0 && (
                                    <div className="text-center py-10 text-slate-500">베팅 데이터가 없습니다.</div>
                                )}
                            </div>
                        )}
                    </div>
        ...
        <div className="flex min-h-screen bg-linear-to-b from-emerald-50 via-white to-white">
            ...
            <div className="w-64 bg-white border-r shadow-sm flex flex-col">
                <div className="p-6 border-b">
                    <h1 className="text-xl font-bold text-slate-800">DB 대시보드</h1>
                </div>
